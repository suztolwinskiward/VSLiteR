# #' Calibrate VS-Lite model parameters using observed climate and ring-width data.
# #'
# #' Given calibration-interval temperature, precipitation, and ring-width data,
# #' and site latitude, \code{estimate_vslite_params} performes a Bayesian parameter
...
# #'
# #' Basic Usage: [T1,T2,M1,M2] = estimate_vslite_params(T,P,phi,RW)
# #'
# #' Basic Usage Inputs:
# #' @param T Monthly calibration-interval temperature, dimension 12 x number of calibration years.
# #' @param P Monthly calibration-interval precipitation, dimension 12 x number of cal. years.
# #' @param phi Site latitude in degrees N.
# #' @param RW Standardized annual calibration-interval ring-width index.
# #' @param errormod Integers 0, 1, or 2 for a white noise model, AR(1) noise model,
# #'        and AR(2) noise model, respectively. Default is 0.
# #' @param gparscalint Indices of input years to use to estimate growth 
# #'        response parameters. Default is all input years.
# #' 
# #'   
...
# 
# estimate_vslite_params <- function(T,P,phi,RW,
#                                    errormod = 0, pt_ests = 'mle', 
#                                    nsamp = 1000, nbi = 200, nchain = 3, convthresh = .1,
#                                    verbose = 1, gparpriors = 'fourbet',
#                                    aT1 = 9, bT1 = (aT1+1)/2,slpT1 = 9, intT1 = 0,
#                                    aT2 = 3.5, bT2 = aT2, slpT2 = 14, intT2 = 10,
#                                    slpM1 = 0.1, intM1 = 0.0, aM1 = 1.5, bM1 = (1-.035/slpM1)*aM1/(.035/slpM1), # .035 is target mean
#                                    slpM2 = 0.4, intM2 = 0.1, aM2 = 1.5, bM2 = (1-(.25-intM2)/slpM2)*aM2/((.25-intM2)/slpM2); # .25 is target mean
#                                    
# ){
#   
#   # INPUT PARSING
...
#   }
#   if(hydroclim == 'M'){ 
#     # if user supplies soil moisture estimate from elsewhere,
...
#     Tt <- To <- Mt <- Mo <- logLdata <- matrix(NA,nsamp+nbi,1);
#     #
#     if( verbose) {
#       cat(paste('Working on chain ',chain,' out of ',nchain,sep=""));
#     }
#     #
#     # Initialize the MCMC:
#     gT <- gM <- NA*T 
#     sim <- 1;
#     # Initialize growth response parameters with draws from the priors.
#     if (gparpriors == 'uniform'){
#       Tt[sim] <- runif(1,aT1,bT1);
#       To[sim] <- runif(1,aT2,bT2);
#       Mt[sim] <- runif(1,aM1,bM1);
#       Mo[sim] <- runif(1,aM2,bM2);
#     }
#     if (gparpriors == 'fourbet'){
#       Tt[sim] <- slpT1*rbeta(aT1,bT1)+intT1;
#       To[sim] <- slpT2*rbeta(aT2,bT2)+intT2;
#       Mt[sim] <- slpM1*rbeta(aM1,bM1)+intM1;
#       Mo[sim] <- slpM2*rbeta(aM2,bM2)+intM2;
#     }
#     #
#     gT[T<Tt[sim]] <- 0;
#     gT[T>To[sim]] <- 1;
...
#       switch gparpriors
#       case 'uniform'
#       Tt(sim) <- Tt_U_aux(Tt(sim-1),T,To(sim-1),gM,t(RW),errorpars,
#               gE,Gterms,aT1,bT1,intwindow,gparscalint);
#       case 'fourbet'
#       Tt(sim) <- Tt_lit_aux(Tt(sim-1),T,To(sim-1),gM,t(RW),errorpars,
...
#     case 'fourbet'
#     To(sim) = To_lit_aux(To(sim-1),T,Tt(sim),gM,t(RW),...
...
# 
# 
# #' \code{errormodel1_aux} does a Metropolis-Hastings step 
# #' 
# #' @param RW = vector of observed annual ring widths
# #' @param Gterms = vector of terms that sum together to give the simulated 
# #' raw ring with index for all months (rows) and all years (columns)
# #' 
# #' SETW 4/5/2013
# #'
# #' @export
# 
# errormodel1_aux <- function(currpars,RW,Gterms,intwindow,cyrs){
#   ########## account for variable integration window:
#   I_0 <- intwindow[1] 
#   I_f = intwindow[2]
#   if (I_0<0){
#     # if we include part of the previous year in each year's modeled growth:
#     startmo <- 13+I_0;
#     endmo <- I_f;
#     prevseas <- cbind(mean(Gterms[startmo:12,],2), Gterms[startmo:12,1:end-1]);
#     Gterms <- Gterms[1:endmo,];
#     Gterms <- rbind(prevseas; Gterms);
#   }else{ 
#     # no inclusion of last year's growth conditions in estimates of this 
#     # year's growth:
#     startmo <- I_0+1
#     endmo <- I_f
#     Gterms <- Gterms[startmo:endmo,]
...
#   phi1prop = runif(1,0,1);
#   tau2prop = runif(1,0,1);
...
#   What <- t((Gamma(cyrs)-Gammabar)/siggamma)
#   #
#   tmp <- makeAR1covmat(phi1prop,tau2prop,Ny)
#   iSigprop <- tmp$iSigma
#   detSigprop <- tmp$det
#   #
#   tmp <- makeAR1covmat(phi1curr,tau2curr,Ny)
#   iSigcurr <- tmp$iSigma
#   detSigcurr <- tmp$det
#   #
#   lphaprop = sqrt(1-tau2prop/(1-phi1prop^2));
#   alphacurr = sqrt(1-tau2curr/(1-phi1curr^2));
#   #
#   logLprop <- -.5*t(t(RW[cyrs])-alphaprop*What)*iSigprop*(t(RW[cyrs])-alphaprop*What);
#   logLcurr <- -.5*(t(t(RW[cyrs])-alphacurr*What)*iSigcurr*(t(RW[cyrs])-alphacurr*What);
...
#                    }
# }
# return(list(pars = pars, logLdata = logLdata))
# }
# 
# ################################################################
...
# ################################################################
# ################################################################
# gelmanrubin92<- function (Nsamp,Nbi,varargin){
